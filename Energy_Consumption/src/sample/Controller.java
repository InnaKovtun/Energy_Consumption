    ...
    void initialize() throws SQLException, ClassNotFoundException {
        ...
        btn_query1.setOnAction(actionEvent -> {
            ...
            try {
                ...
                field_resultQuery.setText(String.valueOf(sum));
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
        ...
        btn_query2.setOnAction(actionEvent -> {
            ...
            field_resultQuery.setText("Максимальна витрата = " + String.valueOf(max));
        ...
        btn_report1.setOnAction(actionEvent -> {
            try {
                ...
                while (res.next()) {
                    report.add("Ім'я: ");
                    report.add(res.getString("clientName")+" ");
                    report.add("Розмір боргу: ");
                    report.add(res.getString("debt")+" ");
        ...
        btn_report2.setOnAction(actionEvent -> {
            try {
                ResultSet res = db.reportTwo();
                List<String> report = new ArrayList<>();
                while (res.next()) {
                    report.add("id Лічильника: ");
                    report.add(res.getString("idCounter")+" ");
                    report.add("Розмір витрати: ");
                    report.add(res.getString("outlay")+" ");
