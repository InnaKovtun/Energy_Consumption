...
//отчёт №1
    public ResultSet reportOne() throws SQLException, ClassNotFoundException {
        String sql = "SELECT clientName, debt FROM clients WHERE debt > 0";
        Statement statement = getDbConnection().createStatement();
        ResultSet res = statement.executeQuery(sql);
        return res;
    }
...
//отчёт №2
    public ResultSet reportTwo() throws SQLException, ClassNotFoundException {
        String sql = "SELECT idCounter, outlay FROM counter_reading WHERE initialDate LIKE '%05%'";
        Statement statement = getDbConnection().createStatement();
        ResultSet res = statement.executeQuery(sql);
        return res;
    }
